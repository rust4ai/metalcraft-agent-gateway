...
/// Run the gateway: build platforms, open the subscriber store, spawn the
/// dispatcher + Discord listener, and serve the HTTP API until shutdown.
///
/// All configuration is read from the environment (see README / `.env.example`).
/// This is the library entrypoint; the binary is a thin wrapper around it.
pub async fn run() {
    dotenvy::dotenv().ok();
